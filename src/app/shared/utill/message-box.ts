...
/**
 * @author Thiago Soares <ths.pereira@gmail.com>
 * @class MessageBox
 * @description Classe para os alertas
 * @date 01/07/2021
 */
...
export class MessageBox {
  ...
  public alertConfirm(msg: string, callback: Function, notConfirm?: Function): void {
    Swal.fire({
      title: msg,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#E95814',
      cancelButtonColor: '#2453A9',
  ...
  public alertConfirmHtml(html: any, callback: Function): void {
    Swal.fire({
      html,
      confirmButtonColor: '#E95814',
  ...
  public alertConfirmMessage(type: TypeMessage, callback: Function): void {
    let msg = 'Mensagem não cadastrada.';
    switch (type) {
      case TypeMessage.Cancel:
        msg =
          'É necessário gravar as alterações realizadas, caso contrário os novos dados serão desconsiderados. Deseja retornar assim mesmo?';
        break;
      case TypeMessage.Continue:
        msg = 'Deseja continuar?';
        break;
    }
    this.alertConfirm(msg, callback);
  }

  public notifySuppress(url: string): void {
    this.navegateUrl(url);
  }

  // public notifyMessage(type: TypeMessage, url?: string): void {
  //   switch (type) {
  //     case TypeMessage.Cancel:
  //       this.notifySuccess('Operação cancelada com sucesso.');
  //       break;
  //     case TypeMessage.Success:
  //       this.notifySuccess('Dados gravados com sucesso!');
  //       break;
  //     default:
  //       this.notifyWarning('Mensagem não cadastrada.');
  //   }
  //   this.navegateUrl(url);
  // }

  public notifyError(msg: object, options?: SweetAlertOptions): void {
    this.createSwal(msg, options, this.notifyError, 'error');
  }

  public notifySuccess(msg: object, options?: SweetAlertOptions): void {
    this.createSwal(msg, options, this.notifySuccess, 'success');
  }

  public notifyWarning(msg: object, options?: SweetAlertOptions): void {
    this.createSwal(msg, options, this.notifyWarning, 'warning');
  }

  public nofityInfo(msg: object, options?: SweetAlertOptions): void {
    this.createSwal(msg, options, this.nofityInfo, 'info');
  }

  private navegateUrl(url: string | undefined): void {
    if (this.router) {
  ...
  private createSwal(msg: any, options: SweetAlertOptions | undefined, fn: Function, icon: SweetAlertIcon) {
    const opt = Object.assign({}, OPTIONS_DEFAULT, options);
    const Toast = Swal.mixin(opt);

    const errorMessage = this.verifyErroCode(msg.code);

    if (Swal.isTimerRunning() || Swal.isVisible()) {
      const name = ('nofity_' + icon).toString();
      clearTimeout(this.time[name]);
      this.time[name] = setTimeout(() => fn.call(this, errorMessage, options), 500);
    } else {
      Toast.fire({
        icon,
        title: errorMessage
  ...
  private verifyErroCode(errorCode: string): string {
    ...
    switch (errorCode) {
      ...
      default:
        return 'Nenhuma mensagem mostrada...';
    }
  }
}
